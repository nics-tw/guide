...
function generateHTML() {
  ...
  exportLink.href = `data:text/html,${encodeURIComponent(
    inlineStyle(previewContainer.innerHTML)
  )}`;
  exportLink.download = `${
    document.querySelector("[data-full-name]").textContent
  }.html`;
...
TITLE:${document.querySelector("[data-title]").textContent}\n`;
  ...
  if (email.textContent) {
    vcard += `EMAIL;TYPE=work:${email.textContent}\n`;
  }
  const phone = document.querySelector("[data-phone]");
  if (phone.textContent) {
    vcard += `TEL:${phone.textContent}\n`;
  }
  const address = document.querySelector("[data-address]");
  if (address.textContent) {
    vcard += `ADR;TYPE=work:${address.textContent};;;;\n`;
  }
  vcard += `END:VCARD`;
  return vcard;
}
